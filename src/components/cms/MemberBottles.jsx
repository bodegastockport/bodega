...
export default function MemberBottles({ member, onBottleCountChange }) {
  ...
  const load = async () => {
    const [{ data: bottleData }, { data: slotData }] = await Promise.all([
      ...
      supabase.from("vault_slots").select("id, section, row_label, column_number, status").eq("member_id", member.id).in("status", ["assigned", "pending_release"]).order("column_number", { ascending: true }).order("row_label", { ascending: true }),
    ]);
