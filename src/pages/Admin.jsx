...
// To set a role: Supabase Dashboard → Authentication → Users → click user → Edit → user_metadata
...
const ALL_TABS    = ["reservations", "cellar", "events", "gallery", "menu", "hire", "contact", "team"];
const TEAM_TABS   = ["reservations", "hire", "contact"];
const TAB_LABELS  = {
  reservations: "Reservations", cellar: "Cellar Club", events: "Events",
  gallery: "Gallery", menu: "Menu", hire: "Hire enquiries", contact: "Messages", team: "Team",
};
...
function TeamManager() {
  ...
  const handleInvite = async (e) => {
    ...
    // Requires the invite-team-member Supabase Edge Function to be deployed.
    // The function calls supabase.auth.admin.inviteUserByEmail(email, { data: { role } })
    // using the service role key (kept securely server-side).
  ...
  const s = { backgroundColor: "#f3f2ee", border: "1px solid #d8d6d0", borderRadius: "6px", fontFamily: "'Courier New', Courier, monospace", fontSize: "13px", padding: "9px 12px", color: "#2e282a", outline: "none", width: "100%" };
  ...
  return (
    <div style={{ fontFamily: "'Courier New', Courier, monospace" }}>
      <div style={{ backgroundColor: "#eceae4", border: "1px solid #d8d6d0", borderRadius: "6px", padding: "24px", maxWidth: "480px" }}>
        ...
        <h2 className="text-sm mb-1" style={{ color: "#2e282a", fontWeight: 400 }}>Add someone to the admin</h2>
        ...
        {sent ? (
          <div>
            <p className="text-sm mb-2" style={{ color: "#2e282a" }}>Invite sent</p>
            <p className="text-xs mb-4" style={{ color: "#777777" }}>They'll receive an email with a link to sign in.</p>
            <button onClick={() => setSent(false)} style={{ padding: "7px 16px", backgroundColor: "transparent", color: "#193c47", border: "1px solid #193c47", borderRadius: "4px", fontFamily: "'Courier New', Courier, monospace", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.08em", cursor: "pointer" }}>
              Invite another
            </button>
          </div>
        ) : (
          <form onSubmit={handleInvite} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-3">
                {[
                  ...
                  <button key={value} type="button" onClick={() => setInviteRole(value)} style={{ flex: 1, padding: "10px 12px", textAlign: "left", backgroundColor: inviteRole === value ? "#193c47" : "#f3f2ee", color: inviteRole === value ? "#f3f2ee" : "#2e282a", border: `1px solid ${inviteRole === value ? "#193c47" : "#d8d6d0"}`, borderRadius: "4px", cursor: "pointer", fontFamily: "'Courier New', Courier, monospace", transition: "all 0.15s" }}>
            ...
            <button type="submit" disabled={inviting || !inviteEmail} style={{ padding: "8px 20px", backgroundColor: "#193c47", color: "#f3f2ee", border: "none", borderRadius: "4px", fontFamily: "'Courier New', Courier, monospace", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.08em", cursor: inviting || !inviteEmail ? "not-allowed" : "pointer", opacity: inviting || !inviteEmail ? 0.6 : 1, display: "inline-flex", alignItems: "center", gap: "6px", transition: "background-color 0.15s" }}>
      ...
      <div style={{ backgroundColor: "#eceae4", border: "1px solid #d8d6d0", borderRadius: "6px", padding: "16px", marginTop: "12px", maxWidth: "480px" }}>
        <p className="text-xs leading-relaxed" style={{ color: "#777777" }}>
          <strong style={{ color: "#2e282a" }}>Team</strong> — reservations, hire enquiries, messages.<br />
          <strong style={{ color: "#2e282a" }}>Full admin</strong> — everything plus Cellar Club, events, gallery, menu and team management.
        </p>
      </div>
    </div>
  );
    ...
    return (
      <div style={{ backgroundColor: "#f3f2ee", fontFamily: "'Courier New', Courier, monospace", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 24px" }}>
        <div style={{ textAlign: "center", maxWidth: "360px" }}>
          ...
          <h1 style={{ fontSize: "18px", color: "#193c47", fontWeight: 400, marginBottom: "10px" }}>You don't have admin access</h1>
          ...
          <a href="/" style={{ display: "inline-block", padding: "8px 20px", backgroundColor: "transparent", color: "#193c47", border: "1px solid #193c47", borderRadius: "4px", fontFamily: "'Courier New', Courier, monospace", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.08em", textDecoration: "none" }}>
            Back to site
          </a>
        </div>
      </div>
    ...
    <div style={{ backgroundColor: "#f3f2ee", fontFamily: "'Courier New', Courier, monospace", minHeight: "100vh" }}>
      <div className="max-w-[1100px] mx-auto px-6 py-10">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            <h1 className="text-xl" style={{ color: "#2e282a", fontWeight: 400 }}>Admin</h1>
            <p className="text-xs mt-1" style={{ color: "#777777" }}>
              Manage all content and bookings for Bodega
              <span style={{ marginLeft: "8px", padding: "2px 8px", backgroundColor: isAdmin ? "#193c47" : "#eceae4", color: isAdmin ? "#f3f2ee" : "#777777", borderRadius: "3px", fontSize: "9px", textTransform: "uppercase", letterSpacing: "0.06em" }}>
        ...
        <div style={{ borderBottom: "1px solid #d8d6d0", marginBottom: "32px" }} className="flex gap-0 overflow-x-auto">
          {availableTabs.map((t) => (
            <button key={t} onClick={() => setTab(t)} style={{ ...TAB_STYLE_BASE, color: tab === t ? "#193c47" : "#777777", borderBottom: tab === t ? "2px solid #193c47" : "2px solid transparent" }}>
              {TAB_LABELS[t]}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="relative">
                ...
                <input style={{ backgroundColor: "#f3f2ee", border: "1px solid #d8d6d0", borderRadius: "6px", fontFamily: "'Courier New', Courier, monospace", fontSize: "13px", padding: "8px 12px 8px 36px", color: "#2e282a", outline: "none", width: "260px" }}
                  placeholder="Search by name, email, phone" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
              <div className="flex gap-2">
                {["list", "calendar"].map((v) => (
                  <button key={v} onClick={() => setView(v)} style={{ padding: "6px 14px", backgroundColor: view === v ? "#193c47" : "transparent", color: view === v ? "#f3f2ee" : "#777777", border: "1px solid", borderColor: view === v ? "#193c47" : "#d8d6d0", borderRadius: "6px", fontFamily: "'Courier New', Courier, monospace", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.06em", cursor: "pointer", transition: "all 0.15s" }}>
                    {v}
                  </button>
                ))}
              </div>
              ...
              <>
                <div style={{ borderBottom: "1px solid #d8d6d0" }} className="flex gap-0">
                  {["today", "upcoming", "past"].map((t) => (
                    <button key={t} onClick={() => setResTab(t)} style={{ ...TAB_STYLE_BASE, fontSize: "11px", color: resTab === t ? "#193c47" : "#777777", borderBottom: resTab === t ? "2px solid #193c47" : "2px solid transparent" }}>
                      {t.charAt(0).toUpperCase() + t.slice(1)} ({counts[t]})
                    </button>
                  ))}
                </div>
                {loading ? (
                  <div className="flex items-center justify-center py-20"><Loader2 className="h-5 w-5 animate-spin" style={{ color: "#193c47" }} /></div>
                ) : filtered.length === 0 ? (
