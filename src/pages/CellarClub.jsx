...
export default function CellarClub() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Find the selected tier to get its Stripe price ID
    ...
    // Step 1: Create a pending member record in Supabase
    ...
    // To wire this up when Stripe is ready:
    // 1. Deploy the create-stripe-checkout Edge Function in Supabase
    // 2. Replace the block below with:
    //
    //   const { data: checkout, error: checkoutErr } = await supabase.functions.invoke('create-stripe-checkout', {
    //     body: { priceId, memberId: member.id, email: form.name, name: form.name }
    //   });
    //   if (checkoutErr || !checkout?.url) {
    //     setError("Payment setup failed. Please try again or contact us.");
    //     setSubmitting(false);
    //     return;
    //   }
    //   window.location.href = checkout.url;
    //
    // For now, show a success state (no payment redirect yet)
    setSubmitting(false);
    setSubmitted(true);
  };
    ...
    <div style={{ backgroundColor: "#f3f2ee", fontFamily: "'Courier New', Courier, monospace" }}>
      <div className="px-6 py-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-xl mb-1" style={{ color: "#193c47", fontWeight: 400 }}>The Cellar Club</h1>
        ...
        {activeTab === "info" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
            <div>
              ...
              <div className="space-y-3 mb-6">
                {[
                  ...
                  <div key={title} style={{ borderLeft: "2px solid #193c47", paddingLeft: "10px" }}>
            ...
            <div>
              <img
                src="https://images.unsplash.com/photo-1569529465841-dfecdab7503b?w=700&q=85"
                alt="Wine vault"
