...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error: err } = await supabase.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: `${window.location.origin}/my-cellar`,
      },
    });
        ...
        {sent ? (
          <div style={{ backgroundColor: "#eceae4", border: "1px solid #d8d6d0", padding: "24px" }}>
            ...
            <p className="text-xs leading-relaxed" style={{ color: "#777777" }}>
              We've sent a sign-in link to <strong style={{ color: "#0A242C" }}>{email}</strong>. Click the link to access your cellar.
            </p>
            <button
