  ...
  return (
    <div style={{ backgroundColor: "#f3f2ee", fontFamily: "'Courier New', Courier, monospace" }}>
      <div className="px-6 py-8">
        <div className="mb-6">
          <p className="text-xs uppercase tracking-widest mb-1" style={{ color: "#777777" }}>What we pour & serve</p>
          <h1 className="text-xl mb-1" style={{ color: "#193c47", fontWeight: 400 }}>Menu</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {/* Wine List */}
          <div>
            <h2 className="text-xs uppercase tracking-widest mb-5" style={{ color: "#777777" }}>Wine list</h2>
            <div className="space-y-6">
              {sections.map(section => {
                ...
                return (
                  <div key={section.id}>
                    <h3 className="text-xs uppercase tracking-widest mb-2" style={{ color: "#193c47", borderBottom: "1px solid #d8d6d0", paddingBottom: "5px" }}>
                      {section.name}
                    </h3>
          ...
          {/* Food */}
          <div>
            <h2 className="text-xs uppercase tracking-widest mb-5" style={{ color: "#777777" }}>Charcuterie & cheese</h2>
            ...
            {foodItems.length > 0 && (
              <div style={{ border: "1px solid #d8d6d0", borderRadius: "4px", overflow: "hidden" }}>
                {foodItems.map((board, i) => (
                  <div key={board.id} style={{ backgroundColor: "#eceae4", padding: "12px 16px", borderTop: i > 0 ? "1px solid #d8d6d0" : "none" }}>
                    <div className="flex items-start justify-between gap-3 mb-0.5">
                      <h3 className="text-xs" style={{ color: "#2e282a", fontWeight: 400 }}>{board.name}</h3>
                      <span className="text-xs shrink-0" style={{ color: "#193c47" }}>{board.price}</span>
                    </div>
                    {board.serves && <p className="text-xs" style={{ color: "#aaa" }}>{board.serves}</p>}
                    {board.description && <p className="text-xs leading-relaxed mt-0.5" style={{ color: "#777777" }}>{board.description}</p>}
                  </div>
                ))}
              </div>
            )}
